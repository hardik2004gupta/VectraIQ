...
class QueryCacheService:
    ...
    def rag_answer_key(self, question: str, cache_context: dict[str, Any] | None = None) -> str:
        ...
        return self._key(
            "rag_answer",
            json.dumps(raw, sort_keys=True) if isinstance(raw, dict) else raw,
        )
    ...
    def get_rag_answer(
        self, question: str, cache_context: dict[str, Any] | None = None
    ) -> dict[str, Any] | None:
    ...
    def clear(self) -> list[str]:
        ...
        if self._redis_client is not None:
            try:
                # Upstash Redis does not expose FLUSHDB via the HTTP client;
                # namespaced SCAN+DEL would require the full Redis API.
                # For now we track the attempt and clear in-memory below.
                cleared.append("redis:skipped(upstash-limitation)")
            except Exception:
